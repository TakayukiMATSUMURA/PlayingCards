...
namespace PlayingCards.Poker
{
    ...
    public class Hand : Hand<Card>
    {
        ...
        {
            var result = $"[{string.Join(",", PocketCards.Select(x => x.ToString()).ToArray())}] {Rank.ToString()}";
            if (Outs.Any())
            {
        ...
        public static ulong Encode(List<Card> cards, int[] rcnt = null, int[] scnt = null)
        {
            ulong result = 0;
            uint bitmap = 0;
            uint mask = 0;
        ...
        internal void SetCalculateResult(Equity.Calculator.Result result)
        {
